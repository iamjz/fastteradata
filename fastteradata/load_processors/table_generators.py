...
def connect_teradata(env, connector):
    ...
    if connector == "pyodbc":
        ...
        session = udaExec.connect(method='odbc', system=env_n, username=usr, password=passw, authentication='LDAP', driver='Teradata Database ODBC Driver 16.20', encoding='iso-8859-1', autocommit=True, transactionMode='Teradata')
...
def prep_load_table(df, table_name, env, db, connector, clear_table):
    ...
    for col, col_type in col_type_zip:
        ...
        if col_type.str == "|O":
            try:
                #t = "varchar({}) ".format(df[col].map(str).map(len).max())
                t = "varchar(250) "
            except:
                t = "varchar(250) "
